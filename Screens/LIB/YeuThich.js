...
const YeuThich = ({ navigation }) => {
  ...
  useEffect(() => {
    ...
    // Filter out the favorite songs (make sure your JSON has a "favorite" field)
    const favoriteSongs = jsonData.filter(song => song.favorite === true);
  ...
  return (
    <View style={styles.container}>
      {favorites.length > 0 ? (
        ...
        <Text style={styles.noFavorites}>No favorite songs available.</Text>
      )}
    </View>
  );
};
